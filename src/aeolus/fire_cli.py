...
def _point(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "behavior": {
            "fire_type": type_name,
            "heading_spread_rate_m_min": float(
                behavior.spread_rate_m_min.item()
            ),
            "fireline_intensity_kw_m": float(
                behavior.fireline_intensity_kw_m.item()
            ),
...
def _render_validation_atlas(
    ...
    for column in range(4):
        ...
        axis.text(
            0.01,
            -0.08,
            f"{int((final[column] != 0).sum()):,} reached cells  ·  "
            f"max ROS {spread[column].max():.1f} m/min",
    ...
    reached = np.asarray(
        [[(snapshot[case] != 0).sum() for snapshot in snapshots] for case in range(4)]
    )
    ...
    timeline_axis.set_title(
        "Resolved growth through time", color="#f1f4f6", fontsize=12, loc="left"
    )
    ...
    behavior_axis.set_title(
        "Reference-table heading spread", color="#f1f4f6", fontsize=12, loc="left"
    )
...
def _suite(args: argparse.Namespace) -> dict[str, Any]:
    ...
    device = _device(args.device)
    state, snapshots, snapshot_minutes, wind = _validation_batch(
        device, args.size, args.minutes
    )
    _synchronize(device)
        ...
        "cases": [
            {
                ...
                "active_crown_cells": int(
                    (fire_type[index] == int(FireType.ACTIVE_CROWN)).sum()
                ),
                "max_spread_rate_m_min": float(
                    state.spread_rate_m_min[index].max().item()
                ),
                "max_intensity_kw_m": float(
                    state.intensity_kw_m[index].max().item()
                ),
            }
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Inspect and validate the Aeolus fire-behavior core"
    )
    ...
    benchmark = subparsers.add_parser(
        "benchmark", help="measure batched tensor-kernel throughput"
    )
    ...
    suite = subparsers.add_parser(
        "validate", help="run idealized cases and render a validation atlas"
    )
