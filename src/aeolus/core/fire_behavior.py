...
class FireBehaviorLookup:
    ...
    def __init__(self) -> None:
        ...
        with as_file(resource) as path, np.load(path, allow_pickle=False) as payload:
            ...
            self.live_herbaceous_grid = payload["moisture_live_herbaceous"].astype(np.float32)
            self.live_woody_grid = payload["moisture_live_woody"].astype(np.float32)
    ...
    def _interp_numpy(
        ...
        axes: tuple[tuple[np.ndarray, np.ndarray, np.ndarray], ...],
    ) -> np.ndarray:
        result = np.zeros_like(fuel_index, dtype=np.float32)
        for corner in range(1 << len(axes)):
            indices: list[np.ndarray] = [fuel_index]
            weight: np.ndarray | float = 1.0
            for dimension, (low, high, fraction) in enumerate(axes):
                upper = bool(corner & (1 << dimension))
                indices.append(high if upper else low)
                weight = weight * (fraction if upper else 1.0 - fraction)
            result += np.asarray(weight * table[tuple(indices)], dtype=np.float32)
        return result
    ...
    def resolve_numpy(
        ...
        *,
        ...
        live_herbaceous = np.broadcast_to(moisture_live_herbaceous, shape).astype(np.float32)
        live_woody = np.broadcast_to(moisture_live_woody, shape).astype(np.float32)
        ...
        herbaceous_axis = _axis_indices(live_herbaceous, self.live_herbaceous_grid)
        woody_axis = _axis_indices(live_woody, self.live_woody_grid)
        ...
        fuel_moisture_axes = (moisture_axis, herbaceous_axis, woody_axis)
        base_ros = self._interp_numpy(
            self.wind_head,
            fuel_index,
            (*fuel_moisture_axes, zero_axis),
        )
        ...
        intensity = np.maximum(0.0, base_intensity * surface_ros / np.maximum(base_ros, 1e-4))
        ...
        if config.enable_crown_fire:
            ...
            critical_intensity = np.power(
                np.maximum(
                    0.01 * canopy_base_height_m * (460.0 + 2600.0 * np.clip(foliar_moisture, 0.3, 2.0)),
                    0.0,
                ),
                1.5,
            )
            ...
            passive_ros = active_ros * np.exp(-active_ros / np.maximum(critical_ros, 1e-5))
            ...
            crown_intensity = intensity + (18_000.0 * canopy_fuel * surface_ros / 60.0)
            ...
            eccentricity = np.where(
                ...
                np.maximum(
                    ...
                    np.sqrt(
                        np.maximum(
                            0.0,
                            1.0 - 1.0 / np.square(np.clip(1.0 + 0.125 * wind * 3.6, 1.0, 100.0)),
                        )
                    ),
                ),
                eccentricity,
    ...
    def _torch_tables(self, device: Any, dtype: Any) -> dict[str, Any]:
        ...
        if key not in self._torch_cache:
            arrays = {
                ...
                "live_herbaceous": self.live_herbaceous_grid,
                "live_woody": self.live_woody_grid,
                "wind": self.wind_grid,
    ...
    @staticmethod
    def _interp_torch(table: Any, fuel: Any, axes: tuple[tuple, ...]) -> Any:
        import torch

        result = torch.zeros_like(fuel, dtype=table.dtype)
        for corner in range(1 << len(axes)):
            indices = [fuel]
            weight: Any = 1.0
            for dimension, (low, high, fraction) in enumerate(axes):
                ...
                indices.append(high if upper else low)
                weight = weight * (fraction if upper else 1.0 - fraction)
            result = result + weight * table[tuple(indices)]
        return result
    ...
    def resolve_torch(
        ...
        *,
        ...
        moisture_live_herbaceous: Any,
        moisture_live_woody: Any,
        ...
        ha = self._torch_axis(
            moisture_live_herbaceous.expand_as(moisture_dead_1h),
            table["live_herbaceous"],
        )
        la = self._torch_axis(
            moisture_live_woody.expand_as(moisture_dead_1h),
            table["live_woody"],
        )
        ...
        fuel_moisture_axes = (ma, ha, la)
        base_ros = self._interp_torch(table["wind_head"], fuel, (*fuel_moisture_axes, za))
        base_i = self._interp_torch(table["wind_intensity"], fuel, (*fuel_moisture_axes, za))
        wind_head = self._interp_torch(table["wind_head"], fuel, (*fuel_moisture_axes, wa))
        wind_back = self._interp_torch(table["wind_back"], fuel, (*fuel_moisture_axes, wa))
        slope_head = self._interp_torch(table["slope_head"], fuel, (*fuel_moisture_axes, sa))
        slope_back = self._interp_torch(table["slope_back"], fuel, (*fuel_moisture_axes, sa))
        ...
        ros = torch.clamp(
            base_ros * (1.0 + phi) * spread_adjustment * config.surface_spread_adjustment,
            0.0,
            config.max_spread_rate_m_min,
        )
        ...
        if config.enable_crown_fire:
            ...
            critical_i = torch.clamp(
                0.01 * canopy_base_height_m * (460.0 + 2600.0 * torch.clamp(foliar_moisture, 0.3, 2.0)),
                min=0.0,
            ).pow(1.5)
            ...
            active = initiated & (active_ros >= critical_ros)
            passive_ros = active_ros * torch.exp(-active_ros / torch.clamp(critical_ros, min=1e-5))
            ...
            crown_eccentricity = torch.sqrt(torch.clamp(1.0 - 1.0 / crown_lw.square(), min=0.0))
            eccentricity = torch.where(
                active,
