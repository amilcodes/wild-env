...
def _cubes(fuel_model: int, moisture: float, wind_m_s: float, slope: float) -> dict:
    values = {
        "slope": slope,
        ...
        "fuel_moisture_live_herbaceous": 0.75,
...
def _behavior(fuel_model: int, moisture: float, wind: float, slope: float) -> tuple:
    cubes = _cubes(fuel_model, moisture, wind, slope)
    head = burn_cell_as_head_fire(cubes, (0, 0, 0), surface_lw_ratio_model="behave")
    back = burn_cell_toward_azimuth(
        cubes, (0, 0, 0), 270.0, surface_lw_ratio_model="behave"
    )
    return (
...
def build(destination: Path) -> None:
    models = np.asarray(
        sorted(
            model
            for model in fuel_models.list_fuel_model_numbers()
            if not 91 <= model <= 99
        ),
        dtype=np.int16,
    )
    wind_shape = (len(models), len(MOISTURE_1H), len(WIND_10M_M_S))
    slope_shape = (len(models), len(MOISTURE_1H), len(SLOPE_TAN))
    ...
    for model_i, model in enumerate(models):
        for moisture_i, moisture in enumerate(MOISTURE_1H):
            for wind_i, wind in enumerate(WIND_10M_M_S):
                values = _behavior(int(model), float(moisture), float(wind), 0.0)
                wind_head[model_i, moisture_i, wind_i] = values[0]
                wind_back[model_i, moisture_i, wind_i] = values[1]
                wind_intensity[model_i, moisture_i, wind_i] = values[2]
                wind_flame[model_i, moisture_i, wind_i] = values[3]
            for slope_i, slope in enumerate(SLOPE_TAN):
                values = _behavior(int(model), float(moisture), 0.0, float(slope))
                slope_head[model_i, moisture_i, slope_i] = values[0]
                slope_back[model_i, moisture_i, slope_i] = values[1]
                slope_intensity[model_i, moisture_i, slope_i] = values[2]
                slope_flame[model_i, moisture_i, slope_i] = values[3]
